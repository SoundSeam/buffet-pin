...
## Fuze and Coke Zero contrast correction — 2026-09-13
User requests outlined logos. Recompose only these two original transparent glasses using the existing official online wordmarks: white Fuze with a dark green outline and white outer edge; red Coke Zero with a strong white script outline and separated white ZERO SUGAR text. Preserve alpha exactly and pixels outside logo ink. New versioned CDN keys keep rollback available. Apply only two image URL changes with transactional audit evidence and expected-old-URL checks; verify CDN bytes and live responsive rendering. Assets and reproducible recipe: /Users/daniel/buffet-pin/output/drinks-outlined-2026-09-13/.

Contrast correction VERIFIED: both uploaded CDN assets are byte-identical to local WebP; original PNG alpha matches exactly. Two transactional item.image.outline audit events accompany the URL replacements; 37 other rows and all other fields unchanged. Live bilingual browser checks pass at 390/768/1440px. These are custom outlined treatments of official web artwork.
